...
async def retrieve_agency_async(session, link):
    async with session.get(link, ssl=ssl_context) as response:  # Disabling SSL verification here
...
links = oig_2021['Link'].unique()  # Only unique links to avoid redundancy
...
merged_districts.plot(column="Action Count", cmap="Blues", legend=True, linewidth=0.8, edgecolor="0.8", ax=ax, legend_kwds={"shrink": 0.5})
ax.set_axis_off()
ax.set_aspect('equal')

ax.set_title("Number of Actions per Judicial District", fontsize=16, pad=20)
...
# Plot with adjusted legend settings
